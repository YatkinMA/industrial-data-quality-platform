...
class SensorDataGenerator:
    ...
    def connect_to_kafka(self):
        ...
        for attempt in range(self.max_retries):
            try:
                logger.info(
                    f"Attempt {attempt + 1}/{self.max_retries} to connect to Kafka at {self.kafka_broker}"
                )
                ...
                logger.warning(
                    f"Kafka not available yet. Waiting {self.retry_delay} seconds..."
                )
    ...
    def run(self, sensor_count=5, interval=1.0):
        ...
        logger.info(
            f"Starting data generation for {sensor_count} sensors with interval {interval}s"
        )
    ...
    try:
        ...
        while True:
            for sensor_id in range(sensor_count):
                ...
                try:
                    ...
                    # Не завершаем работу, продолжаем пытаться
        ...
        time.sleep(5)
        self.run(sensor_count, interval)  # Рекурсивный перезапуск
    finally:
        self.producer.close()
